...
XI = np.zeros(N)
'''COST = np.zeros((N,2))'''
xo=np.array([[1.,1.],[1.,1.],[1.,1.]])
theta=1
LAMBDA= (0.7,0.7,0.7)
XI = (20,20,20)
COST = np.array([[10.,0.0001],[10.,0.0001],[10.,0.0001]])
...
def compute_target_levels(player):
    ...
    y = np.copy((REL[player][:, None]*xo)[np.arange(len(xo))].max(axis=0)) # Find the maxima of x^nuc, x^conv over players except 'player'.
    return y
